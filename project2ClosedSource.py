...
def read_pdf(file_bytes) -> str:
    pdf_bytes = io.BytesIO(
        file_bytes.getbuffer())  # Wrap the uploaded file in a BytesIO stream so pdfplumber can use it.
...
def main():
    ...
    # Incorporate in streamlit
    question = st.text_input("Enter your Question")
    upload_file = st.file_uploader("Upload attachment", type=['txt', 'pdf', 'doc', 'docx', 'html'])

    if st.button("Generate Response"):
        if not question:
            st.warning("Plese enter your question.")
        else:
            context = ""
            if upload_file is not None:
                try:
                    with st.spinner("Reading and processing document..."):
                        context = extract_document_text(upload_file)
                        prompt = f"""
                  You are a helpful assistant. Use the following document as context when answering the question.
                  ...
                  """
                        ...
                        answer = response_llm["message"]["content"]
